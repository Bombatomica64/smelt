...
```text
error[E0609]: no field `value` on type `std::option::Option<(i64, SmeltUnknown)>`
  --> src/sumBy.rs:44:33
   |
44 |     _smelt_tmp_14 = first_entry.value.clone();
   |                                 ^^^^^ unknown field
   |
...
9. **warning** `unused_parens` - 3 occurrences
   ...
   - Examples:
     ...
     - `src/sample.rs:71`
     - `src/take.rs:35`
10. **error** `E0609` - 1 occurrence
   - Message: no field `value` on type `std::option::Option<(i64, SmeltUnknown)>`
   - Examples:
     - `src/sumBy.rs:44`
...
error: could not compile `remeda_smelt_probe` (bin "remeda_smelt_probe") due to 1 previous error; 253 warnings emitted
```
