...
error: could not compile `remeda_smelt_probe` (bin "remeda_smelt_probe" test) due to 276 previous errors
```
...
7. **warning** `unused_parens` - 2 occurrences
   ...
   - Examples:
     - `src/dropWhile.rs:42`
     - `src/sample.rs:71`
8. **error** `E0271` - 1 occurrence
   - Message: expected `{async block@src/funnel_reference_batch_test.rs:120:390: 120:400}` to be a future that resolves to `Result<SmeltUnknown, Box<dyn Error>>`, but it resolves to `SmeltUnknown`
   - Examples:
     - `src/funnel_reference_batch_test.rs:120`
9. **error** `E0277` - 1 occurrence
