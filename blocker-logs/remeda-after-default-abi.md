...
2. **warning** `unused_assignments` - 237 diagnostics
3. **warning** `unused_mut` - 191 diagnostics
4. **warning** `unreachable_code` - 64 diagnostics
5. **warning** `unused_parens` - 36 diagnostics
6. **error** `E0308` - 8 diagnostics
7. **error** `E0599` - 2 diagnostics
8. **error** `no-code` - 1 diagnostic

## Groups

1. **warning** `unused_mut` - 191 occurrences
...
2. **warning** `unreachable_code` - 64 occurrences
   ...
   - Examples:
     ...
     - `src/countBy.rs:70`
3. **warning** `unused_assignments` - 21 occurrences
...
9. **error** `E0308` - 8 occurrences
   - Message: mismatched types
   - Examples:
     - `src/flat.rs:14`
     - `src/heap.rs:94`
     - `src/isDeepEqual.rs:86`
     - `src/mapToObj.rs:8`
     - `src/pipe.rs:79`
...
24. **warning** `unused_parens` - 3 occurrences
   ...
   - Examples:
     ...
     - `src/funnel.rs:188`
     - `src/splitAt.rs:37`
25. **error** `E0599` - 2 occurrences
   - Message: no method named `into_smelt_unknown` found for struct `Rc<RefCell<(dyn FnMut(SmeltUnknown, f64, Vec<SmeltUnknown>) -> SmeltUnknown + 'static)>>` in the current scope
   - Examples:
     - `src/purryFromLazy.rs:26`
     - `src/purryFromLazy.rs:33`
...
489. **warning** `unused_imports` - 1 occurrence
   ...
   - Examples:
     - `src/main.rs:783`
490. **warning** `unused_parens` - 1 occurrence
   - Message: unnecessary parentheses around `return` value
   - Examples:
     - `src/randomBigInt.rs:88`
491. **warning** `unused_parens` - 1 occurrence
...
error: could not compile `remeda_smelt_probe` (bin "remeda_smelt_probe") due to 11 previous errors; 890 warnings emitted
```
