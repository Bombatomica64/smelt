...
## Ordinary Package Import Result Before Fix
...
## Public Source Barrel Result Before Fix
...
The manifest dependency resolver now:

- falls back to `src/index.ts` or `index.ts` when package metadata exposes
  JavaScript build output but TypeScript source exists in the package;
- maps conventional `export * from "./flat"` barrel edges for named imports
  directly to `flat.ts`;
- lowers only requested named-export source modules, avoiding expansion of an
  unrelated package barrel.
...
Both probes build and run successfully after the resolver change.
