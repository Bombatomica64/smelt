...
- `__smelt_module_identity_test`: `failed` - `no test-result line`

```text
error[E0614]: type `f64` cannot be dereferenced
    --> src/main.rs:1598:114
     |
1598 | ...imestamp_millis(*timestamp_ms as i64).map(|date| date.naive_utc()),...
     |                    ^^^^^^^^^^^^^ can't be dereferenced

error[E0433]: failed to resolve: use of unresolved module or unlinked crate `chrono_tz`
    --> src/main.rs:1598:188
     |
1598 | ... timezone_name.parse::<chrono_tz::Tz>()) {
     |                           ^^^^^^^^^ use of unresolved module or unlinked crate `chrono_tz`
     |
help: there is a crate or module with a similar name
     |
1598 -                 if let (Some(mut local), Ok(timezone)) = (chrono::DateTime::<chrono::Utc>::from_timestamp_millis(*timestamp_ms as i64).map(|date| date.naive_utc()), timezone_name.parse::<chrono_tz::Tz>()) {
1598 +                 if let (Some(mut local), Ok(timezone)) = (chrono::DateTime::<chrono::Utc>::from_timestamp_millis(*timestamp_ms as i64).map(|date| date.naive_utc()), timezone_name.parse::<chrono::Tz>()) {
     |
...
```
- `__smelt_module_constant_test`: `failed` - `no test-result line`
...
error: could not compile `remeda_smelt_probe` (bin "remeda_smelt_probe" test) due to 2 previous errors
```
- `__smelt_module_filter_test`: `failed` - `no test-result line`
...
error: could not compile `remeda_smelt_probe` (bin "remeda_smelt_probe" test) due to 2 previous errors
```
- `__smelt_module_zipWith_test`: `failed` - `no test-result line`
...
- `__smelt_module_sumBy_test`: `failed` - `no test-result line`
...
5. **error** `E0433` - 1 diagnostic
6. **error** `E0614` - 1 diagnostic
...
9. **warning** `unused_parens` - 3 occurrences
   ...
   - Examples:
     ...
     - `src/sample.rs:71`
     - `src/take.rs:35`
10. **error** `E0433` - 1 occurrence
   - Message: failed to resolve: use of unresolved module or unlinked crate `chrono_tz`
   - Examples:
     - `src/main.rs:1598`
11. **error** `E0614` - 1 occurrence
