# Generated Rust Test Report

- Cargo manifest: `third_party/remeda/dist-smelt/Cargo.toml`
- Focused runs: `4`
- Guard runs: `2`
- Full suite executed: `false`

## Focused Runs

- `__smelt_module_length_test`: `failed` - `test result: FAILED. 2 passed; 2 failed; 0 ignored; 0 measured; 1785 filtered out; finished in 0.01s`

```text

running 4 tests
.. 2/4
__smelt_module_length_test::test_curried_iterable --- FAILED
__smelt_module_length_test::test_data_first_iterable --- FAILED

failures:

failures:
    __smelt_module_length_test::test_curried_iterable
    __smelt_module_length_test::test_data_first_iterable

test result: FAILED. 2 passed; 2 failed; 0 ignored; 0 measured; 1785 filtered out; finished in 0.01s


thread '__smelt_module_length_test::test_curried_iterable' (910700) panicked at src/length.rs:12:411:
unknown is not array
note: run with `RUST_BACKTRACE=1` environment variable to display a backtrace

thread '__smelt_module_length_test::test_data_first_iterable' (910702) panicked at src/length.rs:12:411:
unknown is not array
error: test failed, to rerun pass `--bin remeda_smelt_probe`
```
- `__smelt_module_first_test`: `failed` - `test result: FAILED. 21 passed; 2 failed; 0 ignored; 0 measured; 1766 filtered out; finished in 0.01s`

```text

running 23 tests
........ 8/23
__smelt_module_first_test::test_readonly_tuple_with_last --- FAILED
............. 22/23
__smelt_module_first_test::test_tuple_with_last_421 --- FAILED

failures:

failures:
    __smelt_module_first_test::test_readonly_tuple_with_last
    __smelt_module_first_test::test_tuple_with_last_421

test result: FAILED. 21 passed; 2 failed; 0 ignored; 0 measured; 1766 filtered out; finished in 0.01s

Error: Custom { kind: Other, error: "expect(...).toBe(...) failed" }
Error: Custom { kind: Other, error: "expect(...).toBe(...) failed" }
error: test failed, to rerun pass `--bin remeda_smelt_probe`
```
- `__smelt_module_last_test`: `passed` - `test result: ok. 4 passed; 0 failed; 0 ignored; 0 measured; 1785 filtered out; finished in 0.00s`
- `__smelt_module_splitAt_test`: `failed` - `test result: FAILED. 2 passed; 2 failed; 0 ignored; 0 measured; 1785 filtered out; finished in 0.00s`

```text

running 4 tests
__smelt_module_splitAt_test::test_data_first_split --- FAILED
.. 3/4
__smelt_module_splitAt_test::test_data_first_split_at_1 --- FAILED

failures:

failures:
    __smelt_module_splitAt_test::test_data_first_split
    __smelt_module_splitAt_test::test_data_first_split_at_1

test result: FAILED. 2 passed; 2 failed; 0 ignored; 0 measured; 1785 filtered out; finished in 0.00s

Error: Custom { kind: Other, error: "expect(...).toStrictEqual(...) failed" }
Error: Custom { kind: Other, error: "expect(...).toStrictEqual(...) failed" }
error: test failed, to rerun pass `--bin remeda_smelt_probe`
```

## Regression Guards

- `__smelt_module_median_test`: `passed` - `test result: ok. 6 passed; 0 failed; 0 ignored; 0 measured; 1783 filtered out; finished in 0.00s`
- `__smelt_module_flat_test`: `passed` - `test result: ok. 28 passed; 0 failed; 0 ignored; 0 measured; 1761 filtered out; finished in 0.01s`

## Compiler Diagnostics

### Rust Diagnostics

- Cargo manifest: `third_party/remeda/dist-smelt/Cargo.toml`
- Cargo check: `passed`
- Errors: `0`
- Warnings: `271`

## Summary By Code

1. **warning** `unused_mut` - 136 diagnostics
2. **warning** `unused_parens` - 93 diagnostics
3. **warning** `unused_unsafe` - 31 diagnostics
4. **warning** `unused_assignments` - 9 diagnostics
5. **warning** `unreachable_code` - 1 diagnostic
6. **warning** `unused_must_use` - 1 diagnostic

## Groups

1. **warning** `unused_mut` - 136 occurrences
   - Message: variable does not need to be mutable
   - Examples:
     - `src/addProp.rs:19`
     - `src/allPass.rs:16`
     - `src/allPass.rs:24`
     - `src/anyPass.rs:16`
     - `src/anyPass.rs:24`
2. **warning** `unused_parens` - 65 occurrences
   - Message: unnecessary parentheses around method argument
   - Examples:
     - `src/funnel.rs:194`
     - `src/funnel.rs:279`
     - `src/funnel.rs:367`
     - `src/funnel.rs:452`
     - `src/funnel.rs:547`
3. **warning** `unused_unsafe` - 31 occurrences
   - Message: unnecessary `unsafe` block
   - Examples:
     - `src/binarySearchCutoffIndex_test.rs:13`
     - `src/debounce.rs:77`
     - `src/debounce.rs:127`
     - `src/debounce.rs:159`
     - `src/debounce.rs:200`
4. **warning** `unused_parens` - 24 occurrences
   - Message: unnecessary parentheses around function argument
   - Examples:
     - `src/countBy.rs:52`
     - `src/dropWhile.rs:39`
     - `src/filter.rs:22`
     - `src/filter.rs:26`
     - `src/find.rs:23`
5. **warning** `unused_assignments` - 4 occurrences
   - Message: value assigned to `rest` is never read
   - Examples:
     - `src/funnel_lodash_debounce_test.rs:111`
     - `src/funnel_lodash_debounce_test.rs:98`
     - `src/funnel_lodash_throttle_test.rs:92`
     - `src/funnel_lodash_throttle_test.rs:79`
6. **warning** `unused_parens` - 2 occurrences
   - Message: unnecessary parentheses around assigned value
   - Examples:
     - `src/dropWhile.rs:42`
     - `src/sample.rs:71`
7. **warning** `unreachable_code` - 1 occurrence
   - Message: unreachable statement
   - Examples:
     - `src/sample.rs:104`
8. **warning** `unused_assignments` - 1 occurrence
   - Message: value assigned to `arg_removed` is never read
   - Examples:
     - `src/purryOrderRules.rs:63`
9. **warning** `unused_assignments` - 1 occurrence
   - Message: value assigned to `arg` is never read
   - Examples:
     - `src/purryOrderRules.rs:62`
10. **warning** `unused_assignments` - 1 occurrence
   - Message: value assigned to `key` is never read
   - Examples:
     - `src/omit.rs:122`
11. **warning** `unused_assignments` - 1 occurrence
   - Message: value assigned to `match_` is never read
   - Examples:
     - `src/stringToPath.rs:33`
12. **warning** `unused_assignments` - 1 occurrence
   - Message: value assigned to `sum` is never read
   - Examples:
     - `src/evolve_test.rs:861`
13. **warning** `unused_must_use` - 1 occurrence
   - Message: unused `Result` that must be used
   - Examples:
     - `src/funnel_reference_batch_test.rs:159`
14. **warning** `unused_parens` - 1 occurrence
   - Message: unnecessary parentheses around `return` value
   - Examples:
     - `src/randomBigInt.rs:93`
15. **warning** `unused_parens` - 1 occurrence
   - Message: unnecessary parentheses around closure body
   - Examples:
     - `src/sample_test.rs:24`

## Cargo Stderr

```text
Checking remeda_smelt_probe v0.1.0 (/home/lollo/Playground/smelt/third_party/remeda/dist-smelt)
    Finished `dev` profile [unoptimized + debuginfo] target(s) in 6.53s
```
