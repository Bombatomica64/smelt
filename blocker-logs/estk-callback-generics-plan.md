...
| Increment 1 — `T` in a callback *and* a value parameter | not started, and now the next behaviour-changing step. `param_type_text` passes `TypeSubstitution::erased()` for the callback's parameter half, so the §1.2 fix is a change to that one argument. `callback_generic_function_still_demotes` in `generics_tests.rs` pins the current gate and is expected to **flip** when this lands. |
