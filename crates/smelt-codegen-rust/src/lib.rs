...
fn emit_source_with_free_function_router(
    ...
    if needs_unknown {
        ...
        writer.line("}");
        writer.blank_line();
        writer.line("impl SmeltPropertyKey for String { fn smelt_array_index(&self) -> Option<u32> { smelt_canonical_array_index(self) } }");
        writer.blank_line();
        writer.line("/// Return the position a newly inserted key takes in a JavaScript own-key order.");
        writer.line("///");
        writer.line("/// `order` keeps its array-index keys as a sorted leading run, so the slot is");
        writer.line("/// found with two binary searches: one for the end of that run, one for the");
        writer.line("/// ascending slot inside it. A non-index key appends. Appending keys in");
        writer.line("/// ascending index order therefore stays linear overall, never quadratic.");
        writer.line("fn smelt_js_key_order_position<K: SmeltPropertyKey>(order: &[K], key: &K) -> usize {");
        writer.line("    let Some(index) = key.smelt_array_index() else { return order.len() };");
        writer.line("    let indexed = order.partition_point(|existing| existing.smelt_array_index().is_some());");
        writer.line("    order[..indexed].partition_point(|existing| existing.smelt_array_index().is_some_and(|existing| existing < index))");
        writer.line("}");
        writer.blank_line();
        ...
        writer.line("    fn new() -> Self { Self { entries: Vec::new(), hashed: ::std::collections::HashMap::new(), unhashed: Vec::new() } }");
        writer.line("    /// Index the freshly pushed slot `slot` under its member hash key.");
        writer.line("    fn remember_slot(&mut self, slot: usize, key: Option<u64>) { match key { Some(key) => self.hashed.entry(key).or_default().push(slot), None => self.unhashed.push(slot) } }");
        ...
        writer.line("    fn forget_slot(&mut self, slot: usize) { self.hashed.retain(|_, slots| { slots.retain(|existing| *existing != slot); for existing in slots.iter_mut() { if *existing > slot { *existing -= 1; } } !slots.is_empty() }); self.unhashed.retain(|existing| *existing != slot); for existing in self.unhashed.iter_mut() { if *existing > slot { *existing -= 1; } } }");
        writer.line("    /// The slots that may hold a member with hash key `key`.");
        ...
        writer.line("    fn position_with_key(&self, value: &T, key: Option<u64>) -> Option<usize> { let store = &*self.store; store.slots(key).iter().copied().find(|slot| Self::same_member(&store.entries[*slot], value)) }");
        ...
        writer.line("    fn insert(&mut self, value: T) -> bool { let key = Self::member_key(&value); if self.position_with_key(&value, key).is_some() { return false; } let store = self.store_mut(); let slot = store.entries.len(); store.entries.push(value); store.remember_slot(slot, key); true }");
        writer.line("    fn remove(&mut self, value: &T) -> bool { match self.position(value) { Some(slot) => { let store = self.store_mut(); store.entries.remove(slot); store.forget_slot(slot); true }, None => false } }");
        ...
        writer.line("    fn insert(&self, key: String, value: SmeltUnknown) -> Option<SmeltUnknown> { if !self.values.borrow().contains_key(&key) { let mut order = self.order.borrow_mut(); let position = smelt_js_key_order_position(&order, &key); order.insert(position, key.clone()); } self.values.borrow_mut().insert(key, value) }");
