...
fn emit_source_with_free_function_router(
    ...
    if needs_unknown {
        ...
        writer.line("}");
        writer.blank_line();
        writer.line("/// Extract the invocable behind an erased callable value.");
        writer.line("///");
        writer.line("/// A dynamically-typed callee is either a bare `SmeltUnknown::Function` or a");
        writer.line("/// callable object exposing a `__smelt_call` function member. Every erased");
        writer.line("/// dispatch site shares this extraction; only the not-callable fallback");
        writer.line("/// differs per site (timer error, `SmeltUnknown::Null`, default callback).");
        writer.line("fn smelt_extract_callable(value: SmeltUnknown) -> Option<::std::rc::Rc<dyn Fn(Vec<SmeltUnknown>) -> Result<SmeltUnknown, Box<dyn std::error::Error>>>> {");
        writer.line("    match value {");
        writer.line("        SmeltUnknown::Function(function) => Some(function),");
        writer.line("        SmeltUnknown::Object(object) => match object.get(\"__smelt_call\") {");
        writer.line("            Some(SmeltUnknown::Function(function)) => Some(function),");
        writer.line("            _ => None,");
        writer.line("        },");
        writer.line("        _ => None,");
        writer.line("    }");
        writer.line("}");
        writer.blank_line();
        writer.line("/// Invoke a dynamically-typed callable value with erased arguments.");
        writer.line("///");
        writer.line("/// A non-callable value yields `Ok(SmeltUnknown::Null)` (matching the");
        writer.line("/// dispatch behavior for erased callees) and a thrown error propagates as");
        writer.line("/// `Err`; call sites decide whether to panic or forward the error.");
        writer.line("fn smelt_call_dynamic(value: &SmeltUnknown, args: Vec<SmeltUnknown>) -> Result<SmeltUnknown, Box<dyn std::error::Error>> {");
        writer.line("    match smelt_extract_callable(value.clone()) {");
        writer.line("        Some(function) => (function)(args),");
        writer.line("        None => Ok(SmeltUnknown::Null),");
        writer.line("    }");
        writer.line("}");
        writer.blank_line();
