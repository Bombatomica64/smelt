...
fn emit_source_with_free_function_router(
    ...
    if needs_unknown {
        ...
        writer.line("    static SMELT_FUNCTION_ITEM_VALUES: ::std::cell::RefCell<::std::collections::HashMap<usize, Box<dyn ::std::any::Any>>> = ::std::cell::RefCell::new(::std::collections::HashMap::new());");
        writer.line("}");
        writer.blank_line();
        ...
        writer.line("}");
        writer.blank_line();
        writer.line("/// Return a stable runtime value for a named function item used as a value.");
        writer.line("///");
        writer.line("/// JavaScript reference identity (`===`) on functions is reference equality, so");
        writer.line("/// every reference to the same named function value must yield the SAME runtime");
        writer.line("/// value. The frontend wraps a function item used as a value in a fresh forwarding");
        writer.line("/// closure per reference, so two references to `func1` would otherwise build two");
        writer.line("/// distinct wrapper `Rc`s and compare unequal. We memoize the wrapper per function");
        writer.line("/// item: the first reference runs `build` and caches its result keyed on the");
        writer.line("/// crate-unique item key; later references return the cached value, so the wrapper");
        writer.line("/// `Rc` is identity-stable. One function item has exactly one signature, so the");
        writer.line("/// per-key `Box<dyn Any>` downcast can never collide.");
        writer.line("fn smelt_function_item_value<T: Clone + 'static>(key: usize, build: impl FnOnce() -> T) -> T {");
        writer.line("    if let Some(existing) = SMELT_FUNCTION_ITEM_VALUES.with(|values| values.borrow().get(&key).and_then(|value| value.downcast_ref::<T>()).cloned()) {");
        writer.line("        return existing;");
        writer.line("    }");
        writer.line("    let value = build();");
        writer.line("    SMELT_FUNCTION_ITEM_VALUES.with(|values| { values.borrow_mut().insert(key, Box::new(value.clone())); });");
        writer.line("    value");
        writer.line("}");
        writer.blank_line();
        ...
        writer.line("fn smelt_is_for_in_object_key(object: &SmeltObject, key: &str) -> bool { key != \"__smelt_date\" && key != \"__smelt_timezone\" && !(object.contains_key(\"__smelt_regexp\") && matches!(key, \"__smelt_regexp\" | \"source\" | \"flags\")) && !(object.contains_key(\"__smelt_error\") && matches!(key, \"__smelt_error\" | \"message\")) }");
        writer
            .line("/// Return whether a record key is visible to JavaScript `for...in` iteration.");
        writer.line("fn smelt_is_for_in_record_key<V>(record: &SmeltRecord<String, V>, key: &str) -> bool { key != \"__smelt_date\" && key != \"__smelt_timezone\" && !(record.contains_key(\"__smelt_regexp\") && matches!(key, \"__smelt_regexp\" | \"source\" | \"flags\")) && !(record.contains_key(\"__smelt_error\") && matches!(key, \"__smelt_error\" | \"message\")) }");
        writer.blank_line();
