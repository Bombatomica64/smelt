...
fn emit_source_with_free_function_router(
    ...
    if needs_unknown {
        ...
        writer.line("pub struct SmeltGenerator<Y, R> {");
        writer.line("    resume: ::std::rc::Rc<::std::cell::RefCell<Box<dyn FnMut() -> SmeltGeneratorResult<Y, R>>>>,");
        writer.line("}");
        writer.line("impl<Y, R> Clone for SmeltGenerator<Y, R> { fn clone(&self) -> Self { Self { resume: self.resume.clone() } } }");
        writer.line("impl<Y, R> ::std::fmt::Debug for SmeltGenerator<Y, R> { fn fmt(&self, formatter: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result { formatter.write_str(\"SmeltGenerator\") } }");
        writer.line("impl<Y, R> SmeltGenerator<Y, R> {");
        writer.line("    /// Hide the producer future type while retaining concrete yield/return types.");
        writer.line("    fn new(resume: impl FnMut() -> SmeltGeneratorResult<Y, R> + 'static) -> Self { Self { resume: ::std::rc::Rc::new(::std::cell::RefCell::new(Box::new(resume))) } }");
        writer.line("    /// Resume execution until the next suspension point or completion.");
        writer.line("    fn resume(&self) -> SmeltGeneratorResult<Y, R> { (self.resume.borrow_mut())() }");
        writer.line("}");
        writer.line("pub struct SmeltAsyncGenerator<Y, R> {");
        writer.line("    resume: ::std::rc::Rc<dyn Fn() -> SmeltFuture<SmeltGeneratorResult<Y, R>>>,");
        writer.line("}");
        writer.line("impl<Y, R> Clone for SmeltAsyncGenerator<Y, R> { fn clone(&self) -> Self { Self { resume: self.resume.clone() } } }");
        writer.line("impl<Y, R> ::std::fmt::Debug for SmeltAsyncGenerator<Y, R> { fn fmt(&self, formatter: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result { formatter.write_str(\"SmeltAsyncGenerator\") } }");
        writer.line("impl<Y, R> SmeltAsyncGenerator<Y, R> {");
        writer.line("    fn new(resume: impl Fn() -> SmeltFuture<SmeltGeneratorResult<Y, R>> + 'static) -> Self { Self { resume: ::std::rc::Rc::new(resume) } }");
        writer.line("    fn resume(&self) -> SmeltFuture<SmeltGeneratorResult<Y, R>> { (self.resume)() }");
        writer.line("}");
        writer.blank_line();
        }
