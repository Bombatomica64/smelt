...
impl FunctionEmitter<'_> {
    ...
    /// throw and the spec's `ToIndex` on the offset; the prelude's checked pair
    /// owns the window bound.
    ///
    ...
    pub(super) fn data_view_access_text(
        ...
        // The offset stays an `f64`: the adapter runs the spec's `ToIndex` on
        // it, which truncates, maps `NaN` to 0 and REJECTS a negative index
        // with a `RangeError`. A `max(0.0) as usize` here would answer byte 0
        // for `getInt16(-1)`, which Node throws on.
        let offset = match accessor_args.first() {
            Some(offset) => format!("({}) as f64", self.numeric_operand_text(offset)?),
            None => "0.0".to_owned(),
        };
