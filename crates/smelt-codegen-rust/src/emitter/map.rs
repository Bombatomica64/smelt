    ...
    pub(super) fn dict_projection_text(
        ...
        match op {
            ...
            smelt_hir::DictProjectionOp::ForInKeys => {
                ...
                if self.mir.types.get(*key_ty) == Some(&Type::String) {
                    // Every string-keyed backing walks the prototype chain through
                    // the same helper; the `SmeltRecord` / `SmeltJsMap` / plain-dict
                    // split that the sibling `Keys` arm needs does not apply here,
                    // so this is one branch rather than three identical ones.
