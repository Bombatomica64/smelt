...
"#,
    ...
    assert!(
        source.contains(
            ".iter().filter(|(key, _)| !key.starts_with(\"__smelt_symbol:\") && !key.starts_with(\"__smelt_proto:\") && !key.starts_with(\"__smelt_method:\") && key != \"__smelt_class\" && key != \"__smelt_proto_object\").map(|(_, value)| value.clone()).collect::<Vec<_>>()"
        ),
        "{source}"
    );
    assert!(
        source.contains(
            ".iter().filter(|(key, _)| !key.starts_with(\"__smelt_symbol:\") && !key.starts_with(\"__smelt_proto:\") && !key.starts_with(\"__smelt_method:\") && key != \"__smelt_class\" && key != \"__smelt_proto_object\").map(|(key, value)| (key.clone(), value.clone())).collect::<Vec<_>>()"
        ),
        "{source}"
    );
}
