...
fn run_generator_fixture(source: &str, crate_name: &str) {
    ...
    // This tier's own keep-variable still works; the shared one is honoured
    // alongside it so `SMELT_KEEP_RUNTIME_SCRATCH=1` keeps every tier's.
    if std::env::var_os("SMELT_KEEP_GENERATOR_FIXTURE").is_none()
        && std::env::var_os("SMELT_KEEP_RUNTIME_SCRATCH").is_none()
    {
