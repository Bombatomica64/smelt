...
use crate::{
    LocalDecl, LocalId, LocalKind, Mir, MirFunction, Operand, Place, Rvalue, Statement, Terminator,
};
...
fn local_is_temp(function: &MirFunction, local: LocalId) -> bool {
    local_decl(function, local).is_some_and(|decl| decl.kind == LocalKind::Temp)
}

/// The declaration of a local, if the index is in range.
fn local_decl(function: &MirFunction, local: LocalId) -> Option<&LocalDecl> {
    function
        .locals
...
fn local_assignment_count(function: &MirFunction, local: LocalId) -> usize {
    function
        .blocks
        .iter()
        .map(|block| {
...
fn local_read_count(function: &MirFunction, local: LocalId) -> usize {
    function
        .blocks
        .iter()
        .map(|block| {
