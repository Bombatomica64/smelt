...
const { format: utilFormat, types: utilTypes } = require("node:util");

const decoratorRecords = [];
const decoratorCallableCaptures = new WeakMap();

/** Record the original callable captured by a decorator-produced wrapper. */
globalThis.__smeltRecordDecoratorResult = function recordDecoratorResult(
  decorator,
  original,
  result,
) {
  if (typeof original !== "function" || typeof result !== "function" || result === original) {
    return result;
  }
  const source = Function.prototype.toString.call(decorator);
  const parameters = source.match(/^[^(]*\(\s*([^,)=\s]+)/u);
  const captureName = parameters ? parameters[1] : "value";
  const resultSource = Function.prototype.toString.call(result);
  const escapedCapture = captureName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  if (!new RegExp(`\\b${escapedCapture}\\b`, "u").test(resultSource)) {
    return result;
  }
  decoratorCallableCaptures.set(result, { captureName, original });
  return result;
};
...
function installGuards(policy) {
  ...
  globalThis.console = {
    log(...values) {
      process.stdout.write(`${utilFormat(...values)}\n`);
    },
    error(...values) {
      process.stderr.write(`${utilFormat(...values)}\n`);
    },
  };
...
/** Resolve one generated byte offset to its original TypeScript position. */
function sourceMappedSpan(moduleRecord, generatedOffset, callableText) {
  const sourceMap = moduleRecord.sourceMap;
  if (!sourceMap || typeof sourceMap.mappings !== "string") return null;
  const prefix = moduleRecord.emittedText.slice(0, generatedOffset);
  ...
  for (let line = 0; line <= generatedLine && line < lines.length; line += 1) {
    let column = 0;
    for (const encoded of lines[line].split(",")) {
      if (!encoded) continue;
      const segment = decodeVlq(encoded);
      if (!segment || segment.length < 4) continue;
      column += segment[0];
      sourceIndex += segment[1];
      originalLine += segment[2];
      originalColumn += segment[3];
      if (line === generatedLine && column <= generatedColumn) {
        mapped = { sourceIndex, originalLine, originalColumn };
      }
    }
  }
  if (!mapped) return null;
  const mappedSource = sourceMap.sources[mapped.sourceIndex];
  if (
    typeof mappedSource !== "string" ||
    path.basename(mappedSource) !== path.basename(moduleRecord.source_path)
  ) {
    return null;
  }
  const sourceLines = moduleRecord.sourceText.split(/\r?\n/u);
  const preceding = sourceLines.slice(0, mapped.originalLine).join("\n");
  const startCharacters =
    preceding.length + (mapped.originalLine === 0 ? 0 : 1) + mapped.originalColumn;
  const line = sourceLines[mapped.originalLine];
  if (typeof line !== "string" || startCharacters > moduleRecord.sourceText.length) {
    return null;
  }
  const callableLines = callableText.split("\n").length;
  let endLine = Math.min(
    mapped.originalLine + callableLines - 1,
    sourceLines.length - 1,
  );
  let endCharacters = sourceLines
    .slice(0, endLine + 1)
    .join("\n").length;
  if (endLine + 1 < sourceLines.length) {
    endCharacters += 1;
  }
  while (
    endLine + 1 < sourceLines.length &&
    !moduleRecord.sourceText.slice(startCharacters, endCharacters).includes("}")
  ) {
    endLine += 1;
    endCharacters =
      sourceLines.slice(0, endLine + 1).join("\n").length +
      (endLine + 1 < sourceLines.length ? 1 : 0);
  }
  return {
    file: moduleRecord.source_path,
    start: Buffer.byteLength(moduleRecord.sourceText.slice(0, startCharacters)),
    end: Buffer.byteLength(moduleRecord.sourceText.slice(0, endCharacters)),
  };
}
...
function callableProvenance(
  ...
  const sourceSpan =
    (resolved &&
      sourceMappedSpan(moduleRecord, resolved.start, resolved.source)) ||
    originalSourceSpan(moduleRecord, qualifiedName);
  ...
  const captures = {};
  const decoratorCapture = decoratorCallableCaptures.get(callable);
  if (decoratorCapture) {
    captures[decoratorCapture.captureName] = serializer.value(
      decoratorCapture.original,
      moduleRecord,
      decoratorCapture.original.name || `${qualifiedName}.wrapped`,
    );
  }
  return {
...
function instrumentDecorators(source) {
  ...
  const instrumentedOpening = `${opening}
    ...
    decorators = decorators.map(function (__smeltDecorator) {
      return function (__smeltValue, __smeltContext) {
        const __smeltResult = __smeltDecorator(__smeltValue, __smeltContext);
        return globalThis.__smeltRecordDecoratorResult(
          __smeltDecorator,
          __smeltValue,
          __smeltResult
        );
...
function classDefinition(classValue, moduleRecord, serializer) {
  ...
  for (const record of recordedTargets) {
    ...
    if (context.kind === "method" && record.descriptor) {
      const callable = record.descriptor.value;
      if (
        typeof callable === "function" &&
        !methods.some(
          (method) =>
            method.name === memberName &&
            method.binding_mode === (context.static ? "class" : "instance"),
        )
      ) {
        methods.push(
          functionDefinition(
    ...
    if (["accessor", "getter", "setter"].includes(context.kind)) {
      let materializedDescriptor = descriptors.find(
        (descriptor) =>
          descriptor.name === memberName &&
          descriptor.is_static === Boolean(context.static),
      );
      if (!materializedDescriptor) {
        const descriptorValue = serializer.value(
          {
            kind: context.kind,
        ...
        descriptors.push(materializedDescriptor);
      }
      const getter =
        context.kind === "getter"
          ? record.descriptor && record.descriptor.get
          : record.descriptor && record.descriptor.get;
      const setter =
        context.kind === "setter"
          ? record.descriptor && record.descriptor.set
          : record.descriptor && record.descriptor.set;
      if (typeof getter === "function") {
        materializedDescriptor.getter = callableProvenance(
          getter,
          moduleRecord,
      ...
      if (typeof setter === "function") {
        materializedDescriptor.setter = callableProvenance(
          setter,
          moduleRecord,
        ...
        materializedDescriptor.write_type = { kind: "dynamic_metadata" };
      }
    }
...
function loadModules(request, events) {
  ...
  Module._extensions[".js"] = function loadInstrumented(module, filename) {
    ...
    const withoutRuntimeSourceMap = source.replace(
      /^\/\/# sourceMappingURL=.*$/gmu,
      "",
    );
    module._compile(instrumentDecorators(withoutRuntimeSourceMap), filename);
  };
  try {
    for (const record of request.modules) {
      const loadedRecord = {
        ...
        sourceMap: record.source_map_path
          ? JSON.parse(fs.readFileSync(record.source_map_path, "utf8"))
          : null,
      };
