...
function sourceMappedSpan(moduleRecord, generatedOffset) {
  ...
  const endCharacters = startCharacters + line.slice(mapped.originalColumn).length;
  return {
...
function callableProvenance(
  ...
  const sourceSpan =
    (resolved && sourceMappedSpan(moduleRecord, resolved.start)) ||
