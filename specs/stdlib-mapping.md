...
| `Number.parseInt(s)` / `parseInt(s)` | `PrimitiveCast::ToInt` with `number` destination | `PrimitiveCast::ToInt` | `s.parse::<i64>().expect(...) as f64` | One string argument | Radix argument and prefix parses | Requires a full Rust integer parse; JS accepts numeric prefixes and returns `NaN` for failures. |
| `Number.parseFloat(s)` / `parseFloat(s)` | `PrimitiveCast::ToFloat` | `PrimitiveCast::ToFloat` | `s.parse::<f64>().expect(...)` | One string argument | Non-string arguments and prefix parses | Requires a full Rust float parse; JS accepts numeric prefixes and returns `NaN` for failures. |
