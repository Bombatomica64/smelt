...
| `Number.parseInt(s)` | `PrimitiveCast::ToInt` with `number` destination | `PrimitiveCast::ToInt` | `s.parse::<i64>().expect(...) as f64` | One string argument | Radix argument, global `parseInt`, prefix parses | Requires a full Rust integer parse; JS accepts numeric prefixes and returns `NaN` for failures. |
| `Number.parseFloat(s)` | `PrimitiveCast::ToFloat` | `PrimitiveCast::ToFloat` | `s.parse::<f64>().expect(...)` | One string argument | Non-string arguments, global `parseFloat`, prefix parses | Requires a full Rust float parse; JS accepts numeric prefixes and returns `NaN` for failures. |
...
| `math.pi` / `math.e` / `math.tau` | `Literal(float)` | `Use(Constant::Float)` | Float literal | Attribute read only | Assignment to module attributes and imported aliases | Values come from Rust `std::f64::consts`. |
...
| `random.randint(a, b)` | `NumericRandomInt` | `NumericRandomInt` | `rand::random_range(a..=b)` | Two integer bounds | Non-integer bounds, keyword args, custom RNG state | Inclusive bounds match Python's supported shape; invalid ranges panic through Rust `rand`. |
| `random.choice(values)` | `ListRandomChoice` | `ListRandomChoice` | `values[rand::random_range(0..values.len())].clone()` | One list argument | Strings, tuples, ranges, sets, custom sequences, keyword args | Empty-list `IndexError` is modeled as a generated panic. |
...
| `str(value)` / `int(value)` / `float(value)` / `bool(value)` | `PrimitiveCast` | `PrimitiveCast` | Direct Rust casts, string parsing, or Python-style bool string spelling | One `bool`, `int`, `float`, or `str` argument | Base arguments, arbitrary object conversion hooks, formatting options | String numeric parse failures panic for now; `str(True)`/`str(False)` preserves Python capitalization. |
...
| `list(tuple_value)` / `set(list_value)` / `set(tuple_value)` | `TupleToList` / `ListToSet` / `TupleToSet` | Matching conversion rvalues | `vec![tuple fields]`, `list.iter().cloned().collect()`, or `HashSet::from([tuple fields])` | Homogeneous tuple or list inputs | Heterogeneous tuple conversion and arbitrary `dict(iterable_pairs)` | Tuple conversions require statically-known homogeneous tuple item types. |
| `tuple(list_value)` | `ListToTuple` | `ListToTuple` | Runtime length check then tuple field clones | Homogeneous list input with statically-sized homogeneous tuple annotation | Unannotated list inputs and heterogeneous tuple annotations | Python's dynamic tuple length is represented only when the destination tuple type is known. Length mismatch panics in generated Rust. |
| `dict(pair_list)` | `ListPairsToDict` | `ListPairsToDict` | `pair_list.iter().cloned().collect::<HashMap<_, _>>()` | `list[tuple[K, V]]` where tuples have exactly two items | Arbitrary iterable pairs, pair lists with non-tuple items, tuple lengths other than two | Duplicate keys keep the last value through Rust `HashMap` collection; order is not preserved. |
