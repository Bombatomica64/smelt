...
fn main() {
    ...
    x = xs.borrow().get({ let len = xs.len() as i64; let index = _smelt_tmp_4.clone() as i64; let normalized = if index < 0 { len + index } else { index }; usize::try_from(normalized).unwrap_or(usize::MAX) }).cloned().unwrap_or(0.0);
    _smelt_tmp_7 = sum + x;
    sum = _smelt_tmp_7;
