...
fn main() {
    ...
    let _ = { println!("{} {}", values.borrow().get({ let len = values.len() as i64; let index = 1.0 as i64; let normalized = if index < 0 { len + index } else { index }; usize::try_from(normalized).unwrap_or(usize::MAX) }).cloned().unwrap_or(0.0).clone(), names.get(&"second".to_owned().clone()).cloned().unwrap_or(String::new()).clone()); };
    return;
}
