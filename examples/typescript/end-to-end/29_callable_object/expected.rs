...
/// `__smelt_class` records the prototype an instance came from and
/// `__smelt_proto_object` holds an `Object.create` result's prototype;
/// JavaScript exposes none of the four as an own property, so deep equality
/// and the matching hash both skip them.
fn smelt_is_own_structural_key(key: &str) -> bool { !key.starts_with("__smelt_proto:") && !key.starts_with("__smelt_method:") && key != "__smelt_class" && key != "__smelt_proto_object" }
