...
fn main() {
    ...
    { let smelt_assign_index = { let normalized = 1.0 as i64; usize::try_from(normalized).expect("negative index out of bounds") }; let smelt_assign_value = 5.0; if smelt_assign_index >= xs.len() { xs.borrow_mut().resize(smelt_assign_index.saturating_add(1), 0.0); } xs.borrow_mut()[smelt_assign_index] = smelt_assign_value; }
    let _smelt_tmp_2: f64 = xs.borrow().get({ let normalized = 0.0 as i64; usize::try_from(normalized).unwrap_or(usize::MAX) }).cloned().unwrap_or_else(|| 0.0) + xs.borrow().get({ let normalized = 1.0 as i64; usize::try_from(normalized).unwrap_or(usize::MAX) }).cloned().unwrap_or_else(|| 0.0);
    let _smelt_tmp_3: f64 = _smelt_tmp_2 + xs.borrow().get({ let normalized = 2.0 as i64; usize::try_from(normalized).unwrap_or(usize::MAX) }).cloned().unwrap_or_else(|| 0.0);
