...
impl IntoSmeltUnknown for Keyed {
    fn into_smelt_unknown(self) -> SmeltUnknown {
...
impl SmeltFromUnknown for Keyed {
    fn smelt_from_unknown(value: SmeltUnknown) -> Self {
...
impl IntoSmeltUnknown for Noted {
    fn into_smelt_unknown(self) -> SmeltUnknown {
        SmeltUnknown::Object(SmeltObject::new(Vec::from([
        ("tag".to_owned(), SmeltUnknown::String(self.tag.into())),
        ("note".to_owned(), SmeltUnknown::String(self.note.into())),
        ])))
    }
}
impl SmeltFromUnknown for Noted {
    fn smelt_from_unknown(value: SmeltUnknown) -> Self {
        let mut result = Self::default();
        if let SmeltUnknown::Object(object) = value {
            if let Some(field) = object.get("tag") {
            ...
            if let Some(field) = object.get("note") {
                result.note = SmeltFromUnknown::smelt_from_unknown(field);
            }
        }
        result
    }
...
impl IntoSmeltUnknown for SmeltUnion2 {
    fn into_smelt_unknown(self) -> SmeltUnknown {
        match self {
            Self::M0(value) => { let smelt_object_value = value; let smelt_struct_value = smelt_object_value.clone(); let mut smelt_object_entries = Vec::new(); smelt_object_entries.push(("tag".to_owned(), SmeltUnknown::String(smelt_object_value.tag.into()))); smelt_object_entries.push(("kind".to_owned(), SmeltUnknown::String(smelt_object_value.kind.into()))); SmeltUnknown::Object(SmeltObject::new(smelt_object_entries)) },
            Self::M1(value) => { let smelt_object_value = value; let smelt_struct_value = smelt_object_value.clone(); let mut smelt_object_entries = Vec::new(); smelt_object_entries.push(("tag".to_owned(), SmeltUnknown::String(smelt_object_value.tag.into()))); smelt_object_entries.push(("note".to_owned(), SmeltUnknown::String(smelt_object_value.note.into()))); SmeltUnknown::Object(SmeltObject::new(smelt_object_entries)) },
        }
    }
}
impl SmeltUnion2 {
...
impl SmeltUnion5 {
    fn from_smelt_unknown(value: SmeltUnknown) -> Self {
        ...
        Self::M1(<SmeltTypedArray as SmeltFromUnknown>::smelt_from_unknown(value.clone()))
    }
}
...
pub(crate) async fn body_text(init: SmeltUnion5) -> Result<String, Box<dyn std::error::Error>> {
    let _smelt_tmp_1: SmeltResponse = SmeltResponse::from_parts(200.0, String::new(), SmeltHeaders::new(), match init.clone() { SmeltUnion5::M0(smelt_body_arm) => SmeltBody::from_text(&smelt_body_arm), SmeltUnion5::M1(smelt_body_arm) => SmeltBody::from_bytes(smelt_body_arm.to_bytes()) });
    let _smelt_tmp_2: SmeltFuture<String> = { let smelt_response = _smelt_tmp_1.clone(); SmeltFuture::from_future(Box::pin(async move { Ok::<_, Box<dyn std::error::Error>>(smelt_response.take_text()?) })) };
    let _smelt_tmp_3: String = _smelt_tmp_2.await?;
    return Ok(_smelt_tmp_3);
}

pub(crate) fn retag(mut value: SmeltUnion2, next: String) -> String {
    match &mut value { SmeltUnion2::M0(smelt_union_arm) => { smelt_union_arm.tag = next.clone(); }, SmeltUnion2::M1(smelt_union_arm) => { smelt_union_arm.tag = next.clone(); } }
    return match &value { SmeltUnion2::M0(smelt_union_arm) => smelt_union_arm.tag.clone(), SmeltUnion2::M1(smelt_union_arm) => smelt_union_arm.tag.clone() }.clone();
}
...
pub(crate) async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    _smelt_tmp_5 = Into::<SmeltList<_>>::into(SmeltList::from({ let smelt_list_items: Vec<f64> = vec![104.0, 105.0, 33.0]; smelt_list_items }));
    _smelt_tmp_6 = SmeltTypedArray::with_elements(SmeltTypedArrayKind::Uint8, &_smelt_tmp_5.to_vec());
