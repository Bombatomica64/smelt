...
struct Ctx<E, P, I> {
    ...
    path: P,
    input: I,
    _smelt_phantom: ::std::marker::PhantomData<(E, P, I)>,
}
impl<E: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static, P: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static, I: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static> Default for Ctx<E, P, I> where E: Default, P: Default, I: Default {
    fn default() -> Self {
        Self {
            env: Default::default(),
...
impl<E, P, I> ::std::fmt::Debug for Ctx<E, P, I> {
    fn fmt(&self, formatter: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        formatter.debug_struct("Ctx").finish_non_exhaustive()
    }
}
impl<E, P, I> Clone for Ctx<E, P, I> where E: Clone, P: Clone, I: Clone {
    fn clone(&self) -> Self {
        Ctx {
            env: self.env.clone(),
...
impl<E, P, I> PartialEq for Ctx<E, P, I> where E: PartialEq, P: PartialEq, I: PartialEq {
    fn eq(&self, other: &Self) -> bool {
        self.env == other.env && self.path == other.path && self.input == other.input
    }
}
impl<E: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static, P: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static, I: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static> IntoSmeltUnknown for Ctx<E, P, I> {
    fn into_smelt_unknown(self) -> SmeltUnknown {
        SmeltUnknown::Object(SmeltObject::new(Vec::from([
        ("env".to_owned(), (self.env).into_smelt_unknown()),
        ("path".to_owned(), (self.path).into_smelt_unknown()),
        ("input".to_owned(), (self.input).into_smelt_unknown()),
        ])))
    }
}
impl<E: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static, P: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static, I: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static> SmeltFromUnknown for Ctx<E, P, I> {
    fn smelt_from_unknown(value: SmeltUnknown) -> Self {
        let mut result = Self::default();
        if let SmeltUnknown::Object(object) = value {
            if let Some(field) = object.get("env") {
                result.env = SmeltFromUnknown::smelt_from_unknown(field);
            }
            if let Some(field) = object.get("path") {
                result.path = SmeltFromUnknown::smelt_from_unknown(field);
            }
            if let Some(field) = object.get("input") {
                result.input = SmeltFromUnknown::smelt_from_unknown(field);
            }
        }
        result
    }
...
fn main() {
    let c: Ctx<String, String, f64>;
    let _smelt_tmp_1: Ctx<String, String, f64> = Ctx::new("root".to_owned(), "/a".to_owned(), 41.0);
    c = _smelt_tmp_1;
    let _smelt_tmp_2: String = none(c.clone());
    let _ = { println!("{}", _smelt_tmp_2); };
    let _smelt_tmp_4: f64 = one(c.clone());
    let _ = { println!("{}", smelt_console_number(_smelt_tmp_4)); };
    let _smelt_tmp_6: f64 = three(c);
    let _ = { println!("{}", smelt_console_number(_smelt_tmp_6)); };
    return;
}

impl<E: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static, P: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static, I: Clone + Default + IntoSmeltUnknown + SmeltFromUnknown + 'static> Ctx<E, P, I> {
    fn new(env: E, path: P, input: I) -> Self {
    let mut this: Self = Ctx { env: Default::default(), path: Default::default(), input: Default::default(), _smelt_phantom: ::std::marker::PhantomData };
    this.env = env.clone();
    ...
    fn __smelt_proto_entries(&self) -> Vec<(String, SmeltUnknown)> {
        ...
        smelt_proto_entries.push(("__smelt_method:where".to_owned(), { let smelt_method: ::std::rc::Rc<dyn Fn(Vec<SmeltUnknown>) -> Result<SmeltUnknown, Box<dyn std::error::Error>>> = ::std::rc::Rc::new({ let smelt_receiver = self.clone(); move |smelt_args: Vec<SmeltUnknown>| { let _ = &smelt_args; let smelt_result = smelt_receiver.where_(); Ok((smelt_result).into_smelt_unknown()) } }); smelt_link_function_identity_key(&smelt_method, smelt_method_identity("Ctx::where")); SmeltUnknown::Function(smelt_method) }));
        smelt_proto_entries
    }
}
...
pub(crate) fn none(c: Ctx<String, String, f64>) -> String {
    let _smelt_tmp_3: String;
    let _smelt_tmp_1: String = c.env.clone() + &":".to_owned();
    let _smelt_tmp_2: String = c.where_();
    _smelt_tmp_3 = _smelt_tmp_1 + &_smelt_tmp_2;
    return _smelt_tmp_3;
}

pub(crate) fn one(c: Ctx<String, String, f64>) -> f64 {
    let _smelt_tmp_1: f64 = c.input * 2.0;
    return _smelt_tmp_1;
}

pub(crate) fn three(c: Ctx<String, String, f64>) -> f64 {
    let _smelt_tmp_1: f64 = c.input + 1.0;
    return _smelt_tmp_1;
}
