...
/// tagged runtime value. The file arm is a `SmeltBlob` whose name is
/// present, which is what a `File` is (see `StdlibClass::File`).
#[derive(Clone, PartialEq, Debug)]
pub enum SmeltFormDataValue {
    /// A text entry.
    Text(String),
    /// A file entry.
    File(SmeltBlob),
...
#[allow(dead_code)]
impl SmeltFormDataValue {
    /// The entry read as text: a text entry verbatim, a file's bytes
    /// decoded as UTF-8. This is what the urlencoded serialization and
    /// `String(value)` observe.
...
/// Build a form entry value from a blob, applying the spec's naming rule.
///
/// A `Blob` that is not a `File` becomes a `File` named `blob`; an
/// explicit `filename` argument replaces whatever name the value has.
/// That falls out of modeling a file as a blob whose name is present.
#[allow(dead_code)]
...
/// A WHATWG `FormData` entry list: ordered, case-sensitive
/// name/value pairs whose values are `string | File`.
#[derive(Clone)]
pub struct SmeltFormData {
    id: usize,
...
impl Default for SmeltFormData { fn default() -> Self { Self::new() } }

#[allow(dead_code)]
impl SmeltFormData {
    /// An empty form with a fresh JS reference identity.
    pub fn new() -> Self { Self { id: smelt_next_object_id(), entries: ::std::rc::Rc::new(::std::cell::RefCell::new(Vec::new())) } }
    /// JS reference identity of this form.
    pub fn id(&self) -> usize { self.id }
    /// Build a form from name/value pairs, in order.
    ...
    /// `set(name, value)`: replace the first entry, drop the rest.
    ///
    /// Position-preserving, like the params list: the surviving entry
    /// stays where the first one was rather than moving to the end.
    ...
    /// `keys()`: one name per ENTRY, in insertion order.
    ///
    /// A duplicated name appears once per entry, because the spec's
    /// iterator walks the entry list rather than a set of names.
    pub fn keys(&self) -> Vec<String> { self.entries.borrow().iter().map(|(name, _)| name.clone()).collect() }
    /// `values()`: entry values in insertion order.
    ...
    /// Parse `application/x-www-form-urlencoded` bytes into a form.
    ///
    /// Delegated to `url::form_urlencoded`, the same decoder
    /// `SmeltUrlSearchParams::from_query` uses. Every entry is text:
    /// this encoding cannot carry a file.
    ...
    /// is a FILE entry, carrying the part's `Content-Type` (the RFC's
    /// default is `text/plain`); any other part is a text entry decoded
    /// as UTF-8. A part with no `name` parameter is skipped, as it names
    /// no form field.
...
/// Split multipart bytes into each part's (header lines, content).
///
/// The delimiter is `--<boundary>`; a part ends at the CRLF before the
/// next delimiter, and the header block ends at the first empty line.
/// The closing `--<boundary>--` delimiter has no part after it, and the
/// preamble before the first delimiter is discarded, both per RFC 7578.
#[allow(dead_code)]
fn smelt_multipart_parts(bytes: &[u8], boundary: &str) -> Vec<(Vec<String>, Vec<u8>)> {
    let delimiter = format!("--{boundary}").into_bytes();
    let mut parts: Vec<(Vec<String>, Vec<u8>)> = Vec::new();
    let Some(mut cursor) = smelt_find_bytes(bytes, &delimiter, 0) else { return parts; };
    loop {
        ...
        let Some(next) = next else { return parts; };
        cursor = next;
    }
}

/// Read one part header's value, matching the name case-insensitively.
#[allow(dead_code)]
...
///
/// RFC 7578 sends `name` and `filename` as quoted strings, so an
/// unquoted form is accepted too but the quoted one is what is written.
#[allow(dead_code)]
fn smelt_multipart_parameter(header: &str, parameter: &str) -> Option<String> {
    for piece in header.split(';') {
        ...
        if !key.trim().eq_ignore_ascii_case(parameter) { continue; }
...
/// Parse body bytes as a form, choosing the parser by `Content-Type`.
#[allow(dead_code)]
fn smelt_form_data_from_body(content_type: Option<String>, bytes: Vec<u8>) -> Result<SmeltFormData, Box<dyn ::std::error::Error>> {
    ...
    if essence == "multipart/form-data" {
        ...
        if boundary.is_empty() {
            return Err(smelt_throw(SmeltUnknown::Object(SmeltObject::new(Vec::from([("__smelt_error".to_owned(), SmeltUnknown::String("TypeError".into())), ("message".to_owned(), SmeltUnknown::String("Content-Type was not one of \"multipart/form-data\" or \"application/x-www-form-urlencoded\".".into())), ("stack".to_owned(), SmeltUnknown::Undefined), ("cause".to_owned(), SmeltUnknown::Undefined)])))));
        }
        return Ok(SmeltFormData::from_multipart(&bytes, &boundary));
    }
    if essence == "application/x-www-form-urlencoded" {
        return Ok(SmeltFormData::from_urlencoded(&bytes));
    }
    return Err(smelt_throw(SmeltUnknown::Object(SmeltObject::new(Vec::from([("__smelt_error".to_owned(), SmeltUnknown::String("TypeError".into())), ("message".to_owned(), SmeltUnknown::String("Content-Type was not one of \"multipart/form-data\" or \"application/x-www-form-urlencoded\".".into())), ("stack".to_owned(), SmeltUnknown::Undefined), ("cause".to_owned(), SmeltUnknown::Undefined)])))));
}

/// Erase a form for a dynamic boundary.
impl IntoSmeltUnknown for SmeltFormData {
    fn into_smelt_unknown(self) -> SmeltUnknown {
...
/// The modeled members of an erased `FormData` record, resolved at run time.
///
...
impl IntoSmeltUnknown for SmeltUnion2 {
    fn into_smelt_unknown(self) -> SmeltUnknown {
        match self {
            ...
            Self::M1(value) => { let smelt_object_value = value; let smelt_struct_value = smelt_object_value.clone(); let mut smelt_object_entries = Vec::new(); smelt_object_entries.push(("name".to_owned(), SmeltUnknown::String(smelt_object_value.name.into()))); smelt_object_entries.push(("__smelt_class".to_owned(), SmeltUnknown::String("Doc".into()))); SmeltUnknown::Object(SmeltObject::new(smelt_object_entries)) },
        }
    }
}
impl SmeltUnion2 {
...
#[derive(Clone)]
pub enum SmeltUnion14 {
    M0(String),
    M1(SmeltBlob),
}
impl IntoSmeltUnknown for SmeltUnion14 {
    fn into_smelt_unknown(self) -> SmeltUnknown {
        match self {
            Self::M0(value) => SmeltUnknown::String(value.into()),
...
impl PartialEq for SmeltUnion14 {
    fn eq(&self, other: &Self) -> bool {
        self.clone().into_smelt_unknown() == other.clone().into_smelt_unknown()
    }
}
impl SmeltFromUnknown for SmeltUnion14 {
    fn smelt_from_unknown(value: SmeltUnknown) -> Self { Self::from_smelt_unknown(value) }
}
impl SmeltJsKeyEq for SmeltUnion14 {
    fn same_js_key(&self, other: &Self) -> bool { self.clone().into_smelt_unknown().same_js_key(&other.clone().into_smelt_unknown()) }
    fn js_key_hash(&self) -> Option<u64> { self.clone().into_smelt_unknown().js_key_hash() }
}
impl ::std::fmt::Debug for SmeltUnion14 {
    fn fmt(&self, formatter: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        ::std::fmt::Debug::fmt(&self.clone().into_smelt_unknown(), formatter)
    }
}
impl Default for SmeltUnion14 {
    fn default() -> Self {
        Self::M0(String::new())
    }
}
...
fn main() {
    let chosen: SmeltUnion2;
    let form: SmeltFormData;
    let entry: Option<SmeltUnion14>;
    let text: Option<SmeltUnion14>;
    let missing: Option<SmeltUnion14>;
    let mut value: SmeltUnion14;
    let _smelt_tmp_19: SmeltFormData;
    let _smelt_tmp_20: SmeltList<String>;
    let _smelt_tmp_21: SmeltBlob;
    let _smelt_tmp_22: ();
    let _smelt_tmp_23: ();
    let _smelt_tmp_24: Option<SmeltUnion14>;
    let _smelt_tmp_25: bool;
    let _smelt_tmp_26: SmeltUnion14;
    let _smelt_tmp_27: SmeltBlob;
    let _smelt_tmp_28: String;
    let _smelt_tmp_29: SmeltUnion14;
    let _smelt_tmp_30: SmeltBlob;
    let _smelt_tmp_31: String;
    let _smelt_tmp_32: SmeltUnion14;
    let _smelt_tmp_33: SmeltBlob;
    let _smelt_tmp_34: f64;
    let _smelt_tmp_37: Option<SmeltUnion14>;
    let _smelt_tmp_38: bool;
    let _smelt_tmp_41: Option<SmeltUnion14>;
    let _smelt_tmp_42: bool;
    let _smelt_tmp_45: SmeltList<SmeltUnion14>;
    let mut _smelt_tmp_46: f64;
    let mut _smelt_tmp_47: f64;
    let mut _smelt_tmp_48: bool;
    let mut _smelt_tmp_49: bool;
    let mut _smelt_tmp_50: SmeltBlob;
    let mut _smelt_tmp_51: String;
    let _smelt_tmp_6: SmeltUnion2 = pick(true);
    let _smelt_tmp_7: String = label(_smelt_tmp_6);
    let _smelt_tmp_8: SmeltUnion2 = pick(false);
    let _smelt_tmp_9: String = label(_smelt_tmp_8);
    let _ = { println!("{} {}", _smelt_tmp_7, _smelt_tmp_9); };
    let _smelt_tmp_11: SmeltUnion2 = branches(true);
    let _smelt_tmp_12: String = label(_smelt_tmp_11);
    let _smelt_tmp_13: SmeltUnion2 = branches(false);
    let _smelt_tmp_14: String = label(_smelt_tmp_13);
    let _ = { println!("{} {}", _smelt_tmp_12, _smelt_tmp_14); };
    let _smelt_tmp_16: SmeltUnion2 = pick(true);
    chosen = _smelt_tmp_16;
    let _smelt_tmp_17: String = label(chosen);
    let _ = { println!("{}", _smelt_tmp_17); };
    _smelt_tmp_19 = SmeltFormData::new();
    form = _smelt_tmp_19;
    _smelt_tmp_20 = Into::<SmeltList<_>>::into(SmeltList::from({ let smelt_list_items: Vec<String> = vec!["hi".to_owned()]; smelt_list_items }));
    _smelt_tmp_21 = SmeltBlob::from_string_parts((_smelt_tmp_20).clone().into(), ("text/plain".to_owned()).clone(), Some(("note.txt".to_owned()).clone()), None);
    _smelt_tmp_22 = form.clone().append(&"doc".to_owned(), smelt_form_data_file_value((_smelt_tmp_21).clone(), None));
    _smelt_tmp_23 = form.clone().append(&"text".to_owned(), SmeltFormDataValue::Text(("plain".to_owned()).clone()));
    _smelt_tmp_24 = form.clone().get(&"doc".to_owned()).map(|smelt_entry| match smelt_entry { SmeltFormDataValue::Text(smelt_text) => SmeltUnion14::M0(smelt_text), SmeltFormDataValue::File(smelt_file) => SmeltUnion14::M1(smelt_file) });
    entry = _smelt_tmp_24;
    _smelt_tmp_25 = matches!(entry.clone(), Some(SmeltUnion14::M1(_)));
    if _smelt_tmp_25 {
    _smelt_tmp_26 = entry.clone().clone().expect("optional value was absent after narrowing");
    _smelt_tmp_27 = match _smelt_tmp_26 { SmeltUnion14::M1(value) => value, _ => unreachable!("union guard selected an excluded member") };
    _smelt_tmp_28 = _smelt_tmp_27.file_name();
    _smelt_tmp_29 = entry.clone().clone().expect("optional value was absent after narrowing");
    _smelt_tmp_30 = match _smelt_tmp_29 { SmeltUnion14::M1(value) => value, _ => unreachable!("union guard selected an excluded member") };
    _smelt_tmp_31 = _smelt_tmp_30.blob_type();
    _smelt_tmp_32 = entry.clone().expect("optional value was absent after narrowing");
    _smelt_tmp_33 = match _smelt_tmp_32 { SmeltUnion14::M1(value) => value, _ => unreachable!("union guard selected an excluded member") };
    _smelt_tmp_34 = _smelt_tmp_33.size();
    let _ = { println!("{} {} {}", _smelt_tmp_28, _smelt_tmp_31, smelt_console_number(_smelt_tmp_34)); };
    } else {
    let _ = { println!("{}", "not a file".to_owned()); };
    }
    _smelt_tmp_37 = form.clone().get(&"text".to_owned()).map(|smelt_entry| match smelt_entry { SmeltFormDataValue::Text(smelt_text) => SmeltUnion14::M0(smelt_text), SmeltFormDataValue::File(smelt_file) => SmeltUnion14::M1(smelt_file) });
    text = _smelt_tmp_37;
    _smelt_tmp_38 = matches!(text, Some(SmeltUnion14::M1(_)));
    if _smelt_tmp_38 {
    let _ = { println!("{}", "unexpectedly a file".to_owned()); };
    } else {
    let _ = { println!("{}", "not a file".to_owned()); };
    }
    _smelt_tmp_41 = form.clone().get(&"absent".to_owned()).map(|smelt_entry| match smelt_entry { SmeltFormDataValue::Text(smelt_text) => SmeltUnion14::M0(smelt_text), SmeltFormDataValue::File(smelt_file) => SmeltUnion14::M1(smelt_file) });
    missing = _smelt_tmp_41;
    _smelt_tmp_42 = matches!(missing, Some(SmeltUnion14::M1(_)));
    if _smelt_tmp_42 {
    let _ = { println!("{}", "unexpectedly a file".to_owned()); };
    } else {
    let _ = { println!("{}", "absent is not a file".to_owned()); };
    }
    _smelt_tmp_45 = Into::<SmeltList<_>>::into(SmeltList::new(form.values().into_iter().map(|smelt_entry| match smelt_entry { SmeltFormDataValue::Text(smelt_text) => SmeltUnion14::M0(smelt_text), SmeltFormDataValue::File(smelt_file) => SmeltUnion14::M1(smelt_file) }).collect::<Vec<_>>()));
    _smelt_tmp_46 = 0.0;
    loop {
    ...
    _smelt_tmp_51 = _smelt_tmp_50.file_name();
    let _ = { println!("{} {}", "file".to_owned(), _smelt_tmp_51); };
    _smelt_tmp_46 = _smelt_tmp_46 + 1.0;
    continue;
    } else {
    let _ = { println!("{} {}", "text".to_owned(), value.into_smelt_unknown()); };
    _smelt_tmp_46 = _smelt_tmp_46 + 1.0;
    continue;
    }
    }
...
impl Doc {
    ...
    let mut this: Self = Doc { name: String::new() };
...
pub(crate) fn pick(flag: bool) -> SmeltUnion2 {
    let mut _smelt_tmp_1: SmeltUnion2;
    if flag {
    let _smelt_tmp_2: Doc = Doc::new("doc".to_owned());
    _smelt_tmp_1 = SmeltUnion2::M1(_smelt_tmp_2);
    } else {
    _smelt_tmp_1 = SmeltUnion2::M0("text".to_owned());
    }
    return _smelt_tmp_1;
}

pub(crate) fn branches(flag: bool) -> SmeltUnion2 {
    if flag {
    let _smelt_tmp_1: Doc = Doc::new("doc".to_owned());
    return SmeltUnion2::M1(_smelt_tmp_1);
    } else {
    return SmeltUnion2::M0("text".to_owned());
    }
}

pub(crate) fn label(value: SmeltUnion2) -> String {
    let _smelt_tmp_2: String;
    let _smelt_tmp_3: String;
    let _smelt_tmp_4: Doc;
    let _smelt_tmp_5: String;
    let _smelt_tmp_1: bool = matches!(value.clone(), SmeltUnion2::M0(_));
    if _smelt_tmp_1 {
    _smelt_tmp_2 = match value.clone() { SmeltUnion2::M0(value) => value, _ => unreachable!("union guard selected an excluded member") };
    _smelt_tmp_3 = "text:".to_owned() + &_smelt_tmp_2;
    return _smelt_tmp_3;
    } else {
    _smelt_tmp_4 = match value.clone() { SmeltUnion2::M1(value) => value, _ => unreachable!("union guard selected an excluded member") };
    _smelt_tmp_5 = "doc:".to_owned() + &_smelt_tmp_4.name.clone();
    return _smelt_tmp_5;
    }
}
