...
fn main() {
    ...
    found = matches.borrow().get({ let len = matches.len() as i64; let index = _smelt_tmp_13.clone() as i64; let normalized = if index < 0 { len + index } else { index }; usize::try_from(normalized).unwrap_or(usize::MAX) }).cloned().unwrap_or(SmeltMatch::default()).clone();
